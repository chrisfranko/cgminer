...
static const char def_conf[] = "ogminer.conf";
...
static struct opt_table opt_config_table[] = {
	...
	OPT_WITH_ARG("--api-description",
		     ...
		     "Description placed in the API status header, default: ogminer version"),
	...
	OPT_WITHOUT_ARG("--benchmark",
			...
			"Run ogminer in benchmark mode - produces no shares"),
	OPT_WITH_ARG("--coin",
...
{
	struct pool *pool = work->pool;
	struct ogminer_pool_stats *pool_stats = &(pool->ogminer_pool_stats);
...
static void calc_diff(struct work *work, double known)
{
	struct ogminer_pool_stats *pool_stats = &(work->pool->ogminer_pool_stats);
	double difficulty;
	uint64_t uintdiff;
...
static bool stale_work(struct work *work, bool share)
{
	 ...
	 * the nearest second */
	getwork_delay = pool->ogminer_pool_stats.getwork_wait_rolling * 5 + 1;
...
static void hash_sole_work(struct thr_info *mythr)
{
	...
	struct ogminer_stats *dev_stats = &(cgpu->ogminer_stats);
	struct ogminer_stats *pool_stats;
	...
	while (likely(!cgpu->shutdown)) {
		...
		do {
			...
			pool_stats = &(work->pool->ogminer_stats);
...
void enable_device(struct cgpu_info *cgpu)
{
	...
	devices[cgpu->ogminer_id = ogminer_id_count++] = cgpu;
...
int main(int argc, char *argv[])
{
	...
	/* ogminer_path is current dir */
	ogminer_path = (char *)alloca(PATH_MAX);
#ifndef _MSC_VER
	s = strdup(argv[0]);
	strcpy(ogminer_path, dirname(s));
	free(s);
	strcat(ogminer_path, "/");
#else
	GetCurrentDirectory(PATH_MAX - 1, ogminer_path);
	strcat(ogminer_path, "\\");
#endif
	...
	if (cnfbuf) {
		...
		switch (fileconf_load) {
			...
			case -1:
				...
				if (use_curses)
					applog(LOG_WARNING, "Start ogminer with -T to see what failed to load.");
				break;
			default:
				break;
		}
	...
	if (opt_devs_enabled) {
		...
		total_devices = ogminer_id_count;
	} else {
		for (i = 0; i < total_devices; ++i)
			enable_device(devices[i]);
	}
	...
	for (i = 0; i < total_devices; ++i)
		devices[i]->ogminer_stats.getwork_wait_min.tv_sec = MIN_SEC_UNSET;
	...
	for (i = 0; i < total_pools; i++) {
		...
		pool->ogminer_stats.getwork_wait_min.tv_sec = MIN_SEC_UNSET;
		pool->ogminer_pool_stats.getwork_wait_min.tv_sec = MIN_SEC_UNSET;
	...
	do {
		...
		if (!pools_active) {
			...
			if (use_curses) {
				halfdelay(150);
				applog(LOG_ERR, "Press any key to exit, or ogminer will try again in 15s.");
				if (getch() != ERR)
