...
## Will ogminer support FPGAs or ASICs?

No. ogminer will only support GPUs. It is bad software design
...
No. `ogminer` keeps a database of the block it's working on to ensure
...
`ogminer` checks for conditions where the primary pool is lagging and
...
`ogminer` may be packaged with other trojan scripts and some antivirus
software is falsely accusing ogminer.exe as being the actual virus,
rather than whatever it is being packaged with. If you had built ogminer
...
[doc/MINING.md](https://github.com/veox/ogminer/blob/master/doc/MINING.md]
...
[doc/BUGS.md](https://github.com/veox/ogminer/blob/master/doc/BUGS.md)
...
offers overclocking tools that can achieve much more than ogminer can do
on Linux. YMMV.


## Can I mine with ogminer on a Mac?

`ogminer` will compile on OSX, but the performance of GPU mining
...
## Are OpenCL kernels from other mining software usable in ogminer?
...
[doc/KERNEL.md](https://github.com/veox/ogminer/blob/master/doc/KERNEL.md)
for an online version.
...
ogminer will automatically detect it and switch to the support as
...
`http://`, ogminer will ONLY try to use stratum protocol mining. The
...
wish ogminer to automatically switch to stratum protocol even if it is
